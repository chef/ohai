...
popen4("ifconfig -a") do |pid, stdin, stdout, stderr|
  ...
  stdout.each do |line|
    if line =~ /^([[:alnum:]|\:|\-]+) \S+ mtu (\d+) index (\d+)/
      ...
      iface[cint]["mtu"] = $2
      iface[cint]["index"] = $3
      if line =~ /flags\=\d+\<((ADDRCONF|ANYCAST|BROADCAST|CoS|DEPRECATED|DHCP|DUPLICATE|FAILED|FIXEDMTU|INACTIVE|LOOPBACK|MIP|MULTI_BCAST|MULTICAST|NOARP|NOFAILOVER|NOLOCAL|NONUD|NORTEXCH|NOXMIT|OFFLINE|POINTOPOINT|PREFERRED|PRIVATE|ROUTER|RUNNING|STANDBY|TEMPORARY|UNNUMBERED|UP|VIRTUAL|XRESOLV|,)+)\>\s/
        flags = $1.split(',')
      else
        flags = Array.new
      end
