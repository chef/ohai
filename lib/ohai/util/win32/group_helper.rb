    ...
    class Win32::GroupHelper
      ...
      def self.windows_root_group_name
        ...
        if convert_result != 0
          ...
          # Use LookupAccountSid rather than WMI because WMI will attempt to include (unneeded) Active Directory
          # groups by querying AD, which is a performance and reliability issue since AD might not be
          # reachable. Additionally, in domains with thousands of groups, the WMI query is very slow,
          # on the order of minutes, even to get the first result. So we use LookupAccountSid which
          # is a purely local lookup of the built-in group, with no need to access AD, and thus no
          # failure modes related to network conditions or query performance.
