...
describe Ohai::System, "Network plugin" do
  ...
  it "should support get_interface_ip" do
    loopback_interface=@ohai[:network][:interfaces].select {|k,v| k if v[:encapsulation] == "Loopback" }.keys.first
    @ohai.get_interface_ip(loopback_interface).should eql("127.0.0.1")
  end
