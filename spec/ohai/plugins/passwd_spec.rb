...
describe Ohai::System, "plugin etc" do
  ...
  if File.exists?("/etc/passwd") && File.exists?("/etc/group")
    it "should include a list of all users" do
      File.should_receive(:readlines).exactly(2).times.and_return(["root:x:0:0:BOFH:/root:/bin/zsh","www:x:800:800:Serving the web since 1970:/var/www:/bin/false"])
      @ohai._require_plugin("passwd")
    ...
    it "should set the available groups" do
      File.should_receive(:readlines).exactly(2).times.and_return(["admin:x:100:root,chef","www:x:800:www,deploy"])
      @ohai._require_plugin("passwd")
